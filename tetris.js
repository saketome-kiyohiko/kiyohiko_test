...
//フリック操作を受け付ける
//指が触れた時の定義
fieldTag.addEventListener("touchstart", function(e) {
...
fieldTag.addEventListener("touchmove", function(e) {
...
// 画面から指が離れたときの処理を定義
fieldTag.addEventListener("touchend", function(e) {
                                        
    e.preventDefault();// スクロール無効化 
    if(startY > moveY+dist){
        minoRoll();
    }else if (startX > moveX && startX > moveX + dist) {
        moveLeft();
    }else if(startX < moveX && startX < moveX - dist ){
        moveRight();
    }else if(startY + dist*2 < moveY ){
        play();
    }
