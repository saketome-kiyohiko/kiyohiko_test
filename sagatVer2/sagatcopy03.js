...
let DamageZone = [];
//             [ 0 ,  1  ,   2  ,    3   ,   4  ,   5   ]
//ダメージゾーン[名 , 始点X, 始点Y, 広さX, 下長さY, ダメージ量]
...
function sagatJumpKnee(){
    if(sagatActionNum>0&&sagatActionNum < 100||sagatActionNum > 105){
        return;
    }
    sagatActionNum = 104;
    sagatX+=1*sr;
    sagatStatImg = sagat_knee;
        attDete[attDete.length] = ["sagatJumpKee", sagatX+1*sr, sagatY+30, sagatX+10*sr, sagatY+70, 1, 1];
...
function removeDete(){
    for(let i=0; i<attDete.length ;i++){
        if(attDete[i][0]=="sagatJumpKee"){//ジャンピングニーが残りやすいので削除
            attDete.splice(i, 1);//判定を削除
            i -= 1;
        }
    }
...
//マップ描画
for(let x=(0+cameraX*-1)/10; x<(canvas.width+cameraX*-1)/10; x++){
...
//テスト用だが、スライムが１匹だけ出てくる関数
function slimeEnemy(hp, chX){
    if(charaSwich){    
        //登場時の初期値
        slime01 = {
            charaImg : new Image(),
    ...
    charBeAttDetes = [[slime01.charaX, slime01.charaY,slime01.charaX+(slime01.charaWidth),slime01.charaY+(slime01.charaHeight)]];
    
    //キャラを描画
    ctx.drawImage(slime01.charaImg,0,0,slime01.charaWidth*2, slime01.charaHeight*2, slime01.charaX+cameraX, slime01.charaY, slime01.charaWidth, slime01.charaHeight);
    returnDete = AttackDetection(charBeAttDetes);//当たり判定関数
    //キャラアクション関数
    //前回のフレームの判定を削除
    for(let i=0; i<DamageZone.length; i++){
        if(DamageZone[i][0]=="slime01"){
            DamageZone.splice(i, 1);
            i-=1;
        }
    }
    
    if(returnDete==null&&slime01.actionCount<90){
        //ノーマル状態のとき
        slime01.charaX -= 1*slime01.reverse;
        slime01.charaImg = slimeImg;
        slime01.actionCount--;
        //ダメージ判定(キャラ画像の半分くらいが判定のとき)
        DamageZone[DamageZone.length] = ["slime01", slime01.charaX+slime01.charaWidth/4, slime01.charaY+slime01.charaHeight/2, slime01.charaX+slime01.charaWidth, slime01.charaY+slime01.charaHeight, 10];
        
    }else{
        slime01.actionCount--;
    }
    if(returnDete!=null){//技食らったリアクション
        slime01.actionCount = 100;
        slime01.charaImg = slimeDamageImg;
        slime01.HP -= returnDete[5];
        console.log(slime01.HP);
        //ノックバック
        slime01.charaX += returnDete[6]*sr*2;
    }
    if(slime01.HP<=1){
        //HPが0になったとき
        slime01.charaHeight -= 6;
        //死ぬ前に判定を消す
        for(let i=0; i< DamageZone.length; i++){
            if(DamageZone[i][0]=="slime01"){
                DamageZone.splice(i, 1);
    ...
    if(slime01.charaY > 700){
        charaSwich = true;
    }
    //キャラが落下判定
    if(slime01.charaY+slime01.charaHeight < field[slime01.charaX+Math.floor(slime01.charaWidth/2)]){
        slime01.charaY += 12;
    }else if(slime01.charaY > field[slime01.charaX+Math.floor(slime01.charaWidth/2)]+slime01.charaHeight){
        slime01.charaY += 12;
    }
}
